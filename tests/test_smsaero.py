...
class TestSmsAero(unittest.TestCase):
    ...
    def test_send_mobile_id(self, mock_request):
        mock_request.return_value = {"success": True}
        result = self.smsaero.send_mobile_id(79031234567, callback_url="https://example.com/callback")
        self.assertEqual(result, {"success": True})
        mock_request.assert_called_once_with(
            "mobile-id/send",
            {"number": 79031234567, "sign": "SMS Aero", "callbackUrl": "https://example.com/callback"}
        )

    @patch.object(SmsAero, "request")
    def test_send_mobile_id_with_sign(self, mock_request):
        mock_request.return_value = {"success": True}
        result = self.smsaero.send_mobile_id(79031234567, callback_url="https://example.com/callback", sign="Test Sign")
        self.assertEqual(result, {"success": True})
        mock_request.assert_called_once_with(
            "mobile-id/send",
            {"number": 79031234567, "sign": "Test Sign", "callbackUrl": "https://example.com/callback"}
        )
    ...
    def test_send_mobile_id_with_callback_url(self, mock_request):
        ...
        result = self.smsaero.send_mobile_id(
            79031234567, callback_url="https://example.com/custom-callback"
        )
        self.assertEqual(result, {"success": True})
        mock_request.assert_called_once_with(
            "mobile-id/send",
